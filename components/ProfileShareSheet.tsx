...
import { Copy, Share2, MessageCircle, Check, X, UserPlus } from "lucide-react";
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: `${displayName} on Drip or Skip`,
          text: `Check out @${username}'s profile on Drip or Skip!`,
          url: shareUrl,
        });
      }
    } catch {
    ...
    <div className="fixed inset-0 z-[90] flex items-end justify-center">
      ...
      <div className="relative w-full max-w-lg bg-bg-raised border border-border-d rounded-t-3xl overflow-hidden animate-in slide-in-from-bottom duration-300">
        ...
        <div className="flex items-center justify-between px-4 pt-4 pb-2">
          <h3 className="font-bold text-sm font-display">Share profile</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-4 py-3 border-b border-border-s">
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <span className="text-xs font-medium text-text-s">Share</span>
            </button>
          </div>
        </div>
